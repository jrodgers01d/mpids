...
from mpi_code_using_pandas import *
...
def main():
  ...
  dataFrame.update(dataFrame.apply(lambda x: pre_process(x), axis = "columns"))
  ...
  #total top words in a file (used for measuring importance of a word in a document)
  total_words = dataFrame['text'].str.split().apply(lambda x: len(x))
  #total_words = dataFrame.apply(lambda x: len(x['text'].split()), axis = "columns")
  print_to_file("\nRemoving everything but top words\n", file_debug)
  #remove everything but top words

  dataFrame.update(remove_non_top_words(dataFrame, top_words.index.values))
  
  
  # transform data (splitting/transposing etc.)
  # columns titles = filenames, rows = words
  dataFrame = dataFrame['text'].str.split(expand=True).transpose()
